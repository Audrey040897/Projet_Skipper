...
def collate_t1t2(batch: list) -> dict:
    ...
    images = [resize_img(item['image']) for item in batch]
...
class SkipperCNN(nn.Module):
    ...
    def __init__(self, in_channels: int = 4, base_ch: int = 32,
        ...
        self.head_t2 = nn.Sequential(
            ...
            nn.Linear(128, 1),         # valeur brute → post-process en mètres
        )
...
def evaluate(model: nn.Module, loader: DataLoader,
    ...
    for batch in loader:
        ...
        # T2 : uniquement sur les échantillons avec conduite
        lbl_t2 = batch['label_t2'].numpy()
        prd_t2 = out['pred_t2'].squeeze(1).cpu().numpy()
        mask   = lbl_t2 >= 0
        if mask.sum() > 0:
...
if __name__ == '__main__':
    ...
    # ⚠️  HYPERPARAMÈTRES
    # ══════════════════════════════════════════════════════════════
    N_EPOCHS   = 100   # 100 époques — laisser tourner toute la nuit sur CPU
    BATCH_SIZE = 4     # 4 avec MAX_IMG_SIZE=256, réduire à 2 si erreur mémoire
    LR         = 3e-4
    LAMBDA_T2  = 0.5
    # ══════════════════════════════════════════════════════════════
