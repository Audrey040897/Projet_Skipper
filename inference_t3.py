...
def preprocess_npz(file_path, img_size=IMG_SIZE):
    ...
    tensor = torch.tensor(data_resized).permute(2, 0, 1).unsqueeze(0)
    return tensor
...
def predict(file_path):
    ...
    Retourne : 0 (intensité insuffisante) ou 1 (intensité suffisante)
    """
    ...
    print(f"Prédiction : {pred}  ({'Intensité suffisante' if pred == 1 else 'Intensité insuffisante'})")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage   : python inference_task3.py <chemin_vers_fichier.npz>")
        print("Exemple : python inference_task3.py Data_SNDT/sample_00015_perfect_straight_clean_field.npz")
        sys.exit(1)
